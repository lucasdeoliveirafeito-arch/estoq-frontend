...
export default function CatalogoProdutos() {
  ...
  const adicionarProduto = async () => {
    ...
    try {
      ...
      setNovoProduto({ nome: '', codigo: '', categoria: '', unidade: '', preco_custo: '' });
      carregarProdutos(); // Atualiza lista após salvar
    } catch (error) {
  ...
  const renderProduto = ({ item }) => (
    <View style={styles.card}>
      <Image
        source={{ uri: item.imagem || 'https://cdn-icons-png.flaticon.com/512/706/706164.png' }}
        style={styles.imagem}
      />
      <Text style={styles.nome}>{item.nome}</Text>
      <Text style={styles.codigo}>SKU: {item.codigo || '—'}</Text>
      <Text style={styles.categoria}>{item.categoria}</Text>
      <Text style={styles.preco}>R$ {item.preco_custo?.toFixed(2)}</Text>
    </View>
  );
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.botaoNovo} onPress={() => setModalVisible(true)}>
          ...
          <Text style={styles.textoNovo}>Novo Produto</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalConteudo}>
            ...
            <TextInput
              ...
              onChangeText={(t) => setNovoProduto({ ...novoProduto, categoria: t })}
            />
            <TextInput
              placeholder="Unidade (kg, L, un...)"
              style={styles.modalInput}
              value={novoProduto.unidade}
              onChangeText={(t) => setNovoProduto({ ...novoProduto, unidade: t })}
            />
            <TextInput
              ...
              onChangeText={(t) => setNovoProduto({ ...novoProduto, preco_custo: t })}
            />
